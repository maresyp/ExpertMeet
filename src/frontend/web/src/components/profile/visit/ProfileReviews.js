...
import { Avatar, Button, CircularProgress, Divider, Paper, Rating, TextField, Typography } from '@mui/material';
...
function ProfileReviews({ profileData }) {
    ...
    const {
        ...
        queryKey: ['VisitReviews', ordering],
        queryFn: ({ pageParam = 1 }) => fetchReviews({ queryKey: ['VisitReviews', pageParam, ordering] }),
    ...
    const addReview = async ({ formData }) => {
        const response = await fetch(`http://127.0.0.1:8080/api/profile/add_review/${profileData.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
    ...
    const { mutate: addReviewMutation, isLoading: addReviewPending, error: addReviewError } = useMutation({
        mutationFn: (formData) => addReview({ formData }),
        onSuccess: (data) => {
            showAlert('Dodano recenzję', 'success');
            resetQuery();
        },
        onError: (error) => {
            console.error('Failed to add review', error);
            showAlert('Nie udało się dodać recenzji', 'error')
        }
    });
    ...
    const handleSubmit = (event) => {
        ...
        const formData = {
            rating: reviewRating,
            content: form_data.get('review-content'),
        }
